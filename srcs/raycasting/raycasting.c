...
/*   Updated: 2023/07/11 13:55:46 by gansard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	raycasting(t_config *config)
{
	...
	printf("Initial player position: (%f, %f)\n", config->p_pos_x, config->p_pos_y);
	printf("Initial player direction: (%f, %f)\n", config->dirx, config->diry);
	...
	while (iter_x < WINDOW_WIDTH)
	{
		...
		if(iter_x < (WINDOW_WIDTH + 10) / 2)
		{
			printf("\nIteration: %d\n", iter_x);
			printf("Line height: %d\n", rc.line_height);
			printf("Drawing start/end: %d/%d\n", rc.draw_start, rc.draw_end);
		}
		process_rc(&rc, config);
